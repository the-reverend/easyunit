...
namespace easyunit
{
	...
	*/
	...
	public:
		...
		*/
		Test( const std::string& testCaseName, const std::string& testName );
		...
		*/
		TestCase* getTestCase( void ) const;
		...
		*/
		TestPartResult* getTestPartResult( void ) const;
		...
		*/
		int getFailuresCount( void ) const;
		...
		*/
		int getSuccessesCount( void ) const;
		...
		*/
		const std::string& getTestCaseName( void ) const;
...
#define TO_STRING_EQUALS_F(expected,actual) \
 ToString("Expected : ") + ToString(expected) + ToString(" but Actual : ") + ToString(actual)

#define TO_STRING_EQUALS_S(expected,actual) \
 ToString(expected) + ToString(" == ") + ToString(actual)

#define TO_S_E_DELTA_F(expected,actual,delta) \
 ToString("Expected : ") + ToString(expected) + ToString(" but Actual : ") + ToString(actual) + ToString(" with delta = ") + ToString(delta)

#define TO_S_E_DELTA_S(expected,actual,delta) \
 ToString(expected) + ToString(" == ") + ToString(actual) + ToString(" with delta = ") + ToString(delta)
...
#define ASSERT_EQUALS(expected,actual) \
 ...
 addTestPartResult(new easyunit::TestPartResult(this, __FILE__,__LINE__,TO_STRING_EQUALS_F(#expected,#actual),failure)); return; \
	...
	* If the two parameters are not equals, a failure is generated.
	*
	* Parameters must be primitive data types or ToString (custom type) must
	* be overloaded.
	*
	* @see easyunit::std::string
	* @param expected Expected value
	* @param actual Actual value to be compared
	*/
...
#define FAIL_M(text) \
 { addTestPartResult(new TestPartResult(this, __FILE__, __LINE__,text,failure)); return; }
	...
	/**
	* Asserts that a expression will throw the exceptionType.
	* If the condition is not true, a failure is generated.
	* @param exceptionType exception to fullfill for the assertion to pass
	*/
#define ASSERT_THROW(expression, exceptionType) \
 ...
 addTestPartResult(new easyunit::TestPartResult(this, __FILE__,__LINE__,#expression,success)); \
 correctExceptionThrown = true; \
 } catch ( ... ) { \
 addTestPartResult(new easyunit::TestPartResult(this, __FILE__,__LINE__, #expression,error)); return; \
 } \
 if( !correctExceptionThrown ){ \
 addTestPartResult(new easyunit::TestPartResult(this, __FILE__,__LINE__, #expression,failure)); \
 }}

	/**
	* Asserts that a expression will not throw the exceptionType.
	* If the condition is not true, a failure is generated.
	* @param exceptionType exception to fullfill for the assertion to pass
	*/
#define ASSERT_NOTHROW(expression, exceptionType) \
 { \
 try { \
 expression; \
 } catch ( const exceptionType & ) { \
 addTestPartResult(new easyunit::TestPartResult(this, __FILE__,__LINE__, #expression,failure));return; \
 } catch ( ... ) { \
 addTestPartResult(new easyunit::TestPartResult(this, __FILE__,__LINE__, #expression,error)); return; \
 } \
 addTestPartResult(new easyunit::TestPartResult(this, __FILE__,__LINE__, #expression,success)); \
	...
	* @param message Message that will be displayed if this assertion fails
	*/
#define ASSERT_THROW_M(expression,exceptionType,message) \
 { bool correctExceptionThrown = false; \
 try { \
 expression; \
 } catch ( const exceptionType & ) { \
 ...
 if( !correctExceptionThrown ){ \
 addTestPartResult(new easyunit::TestPartResult(this, __FILE__,__LINE__, message,failure)); \
 }}
	...
	* @param message Message that will be displayed if this assertion fails
	*/
#define ASSERT_NOTHROW_M(expression,exceptionType,message) \
 { \
 try { \
 expression; \
 } catch ( const exceptionType & ) { \
...
#define DECLARE(testCaseName) \
 ...
 { public: testCaseName##Declare##Test(const std::string& testCaseName, const std::string& testName) : Test (testCaseName , testName) {} \
