...
int TestResult::getTotalErrorCount() const
{
	return totalErrors_;
}

int TestResult::getTotalRunCount() const
{
	return testRanCount_;
}

int TestResult::getTotalFailureCount() const
{
	return totalFailures_;
}

int TestResult::getTotalSuccessCount() const
{
	int count = 0;
	if ( testCaseCount_ )
	{
		...
		{
			count += testCases_[i].getSuccessesCount();
		}
	}
